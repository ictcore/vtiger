...
class ICTBroadcast_List_View extends Vtiger_Index_View {
	...
	function preProcess(ICTBroadcast_Request $request, $display=true) {
	...
	function process (ICTBroadcast_Request $request) {
		$viewer = $this->getViewer ($request);
		$moduleName = $request->getModule();
	 ...
	 */
	public function initializeListViewContents(ICTBroadcast_Request $request, Vtiger_Viewer $viewer) {
		$moduleName = $request->getModule();
		$cvId = $this->viewName;
