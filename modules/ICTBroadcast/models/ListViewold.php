 ...
 */
class ICTBroadcast_ListView_Model extends Vtiger_Base_Model {
	...
	public function getSideBarLinks($linkParams) {
		...
		$listLinks = ICTBroadcast_Link_Model::getAllByType($this->getModule()->getId(), $listLinkTypes);
	...
	public function getListViewLinks($linkParams) {
		...
		$links = ICTBroadcast_Link_Model::getAllByType($moduleModel->getId(), $linkTypes, $linkParams);

		$basicLinks = $this->getBasicLinks();

		foreach($basicLinks as $basicLink) {
			$links['LISTVIEWBASIC'][] = ICTBroadcast_Link_Model::getInstanceFromValues($basicLink);
		}

		$advancedLinks = $this->getAdvancedLinks();

		foreach($advancedLinks as $advancedLink) {
			$links['LISTVIEW'][] = ICTBroadcast_Link_Model::getInstanceFromValues($advancedLink);
		}
		...
		if($currentUserModel->isAdminUser()) {
			...
			foreach($settingsLinks as $settingsLink) {
				$links['LISTVIEWSETTING'][] = ICTBroadcast_Link_Model::getInstanceFromValues($settingsLink);
			}
		}
	...
	public function getListViewMassActions($linkParams) {
		...
		$modCommentsModel = ICTBroadcast_Module_Model::getInstance('ModComments');
		...
		foreach($massActionLinks as $massActionLink) {
			$links['LISTVIEWMASSACTION'][] = ICTBroadcast_Link_Model::getInstanceFromValues($massActionLink);
		}
	...
	public function getListViewHeaders() {
		...
		foreach($headerFields as $fieldName => $webserviceField) {
			...
			if(count($matches) > 0) {
				...
				$referenceModuleModel = ICTBroadcast_Module_Model::getInstance($referenceModule);
				$referenceFieldModel = ICTBroadcast_Field_Model::getInstance($referenceFieldName, $referenceModuleModel);
				...
				$fieldInstance = ICTBroadcast_Field_Model::getInstance($fieldName,$module);
	 ...
	 * @param ICTBroadcast_Paging_Model $pagingModel
	...
	public function getListViewEntries($pagingModel) {
		...
		if(!empty($orderBy)){
			...
			if($orderByFieldModel && ($orderByFieldModel->getFieldDataType() == ICTBroadcast_Field_Model::REFERENCE_TYPE ||
					$orderByFieldModel->getFieldDataType() == ICTBroadcast_Field_Model::OWNER_TYPE)){
	...
	public static function getInstance($moduleName, $viewId='0', $listHeaders = array()) {
		...
		$modelClassName = ICTBroadcast_Loader::getComponentClassName('Model', 'ListView', $moduleName);
	...
	public static function getInstanceForPopup($value) {
		...
		$modelClassName = ICTBroadcast_Loader::getComponentClassName('Model', 'ListView', $value);
		$instance = new $modelClassName();
		$moduleModel =ICTBroadcast_Module_Model::getInstance($value);
