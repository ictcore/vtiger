...
class Vtiger_MassActionAjax_View extends Vtiger_IndexAjax_View {
	function __construct() {
		...
		$this->exposeMethod('startbroadcasting');
	 ...
	 * @param Vtiger_Request $request
	 */
	function startbroadcasting(Vtiger_Request $request) {
...
//echo "<pre>";print_r($dsd);
	...
	$arguments = array('campaign_id'=>'', 'usr_id'=>1);
     ...
     if($result[0] == true) {
      ...
      // $res = json_encode($data);
       //$rest = json_decode( json_encode($data),true);
     // echo "<pre>"; print_r($data);
       /*$campaign_type = array();
      foreach($data as $list){
       //echo "<pre>"; print_r($list['info']);

      	$campaign_type[] = $list->campaign_id;
      	$campaign_type_name[] = $list->name;
       }
       $resul = array_merge($campaign_type,$campaign_type_name);
...
function broadcast_api($method, $arguments = array()) {
      ...
      $service_url  = '202.142.186.26/rest'; // <=== URL for ICTBroadcast REST APIs
