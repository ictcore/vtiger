...
{strip}
	<div class="col-sm-12 col-xs-12">
		<div class="container-fluid" id="AsteriskServerDetails">
			<input type="hidden" name="module" value="ICTBroadcast"/>
			<input type="hidden" name="action" value="SaveAjax"/>
			<input type="hidden" name="parent" value="Settings"/>
			<input type="hidden" class="recordid" name="id" value="ICTBroadcast">
			<div class="widget_header row">
				<div class="col-sm-8"><h4>{vtranslate('ICTBroadcast', $QUALIFIED_MODULE)}</h4></div>
				{assign var=MODULE_MODEL value=Settings_ICTBroadcast_Module_Model::getCleanInstance()}
				<div class="col-sm-4">
					<div class="clearfix">
						<div class="btn-group pull-right editbutton-container">
							<button class="btn btn-default editButton" data-url="{$MODULE_MODEL->getEditViewUrl()}&mode=showpopup&id=ICTBroadcast" title="{vtranslate('LBL_EDIT', $QUALIFIED_MODULE)}">{vtranslate('LBL_EDIT',$QUALIFIED_MODULE)}</button>
						</div>
					</div>
				</div>
			</div>
